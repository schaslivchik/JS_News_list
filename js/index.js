...
const itemList = news.map((e) => createListNewItem(e));

function createListNewItem({ title, headerBgSrc, category, body, date }) {
  const listNewsContainer = document.querySelector(".list-news");
  const listNewsItem = document.createElement("li");
  listNewsItem.classList.add("list-news-item");
  listNewsContainer.append(listNewsItem);

  listNewsItem.append(
    createInnerNewsContainer(
      createBtnsPanel(),
      createTitleNewsBlockH3(title),
      headerBgSrc
    ),
    createdInnerTextContainer(
      createdTitleCategory(category),
      createdBodyTxt(body),
...
function createInnerNewsContainer(btnsPanel, titleNewsBlockH3, headerBgSrc) {
  const innerNewsContainer = document.createElement("div");
  innerNewsContainer.classList.add("inner-news-container");
  innerNewsContainer.append(btnsPanel, titleNewsBlockH3);
  innerNewsContainer.style.backgroundImage = `url(${headerBgSrc})`;
...
function createTitleNewsBlockH3(title) {
  const titleNewsBlockH3 = document.createElement("h3");
  titleNewsBlockH3.classList.add("title-news-block");
  titleNewsBlockH3.textContent = title;
  return titleNewsBlockH3;
}

function createdInnerTextContainer(titleCategory, bodyTxt, newsDate) {
  const innerTextContainer = document.createElement("div");
  innerTextContainer.classList.add("inner-text-container");
  innerTextContainer.append(titleCategory, bodyTxt, newsDate);
  return innerTextContainer;
}

function createdTitleCategory(category) {
...
function createdBodyTxt(body) {
  const bodyTxt = document.createElement("p");
  bodyTxt.classList.add("body-text");
  bodyTxt.textContent = body;
  return bodyTxt;
}

function createdNewsDate(date) {
